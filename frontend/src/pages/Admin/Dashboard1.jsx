...
function Dashboard1() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const response = await axiosInstance.get(API_PATHS.TASK.DASHBOARD_DATA);
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', 'tasks_report.xlsx'); // or .xlsx depending on backend
  ...
  // We don't need the generic COLORS array anymore for the pie chart, but might for bar chart if needed.
  // Let's keep a generic one for Bar Chart or fallback.
      ...
      <div className="flex-1 p-8 ml-64 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            ...
            color="bg-green-500" // Corresponds to #00C49F
          />
          <StatCard
            title="Pending"
            value={statistics.pendingtasks}
            icon={<FaClock className="text-white text-2xl" />}
            color="bg-yellow-500" // Corresponds to #FFBB28
          />
          <StatCard
            title="Overdue"
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    fill="#8884d8"
                    dataKey="value"
                    paddingAngle={5} // Separate slices
                  >
                    {statusData.map((entry, index) => (
                      // entry.name is "Pending", "In Progress" etc. We need the key to look up color.
                      // But wait, statusData lost the original key 'pending'.
                      // We can recover it or just check the name.
